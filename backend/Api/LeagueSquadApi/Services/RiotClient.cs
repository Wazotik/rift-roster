...
namespace LeagueSquadApi.Services
{
    public class RiotClient : IRiotClient
    {
        ...
        {
            var urlCore = $"{http.BaseAddress}riot/account/v1/accounts/by-riot-id/{Uri.EscapeDataString(gameName)}/{Uri.EscapeDataString(tagLine)}";
            ...
            var urlRegion = $"{http.BaseAddress}riot/account/v1/region/by-game/lol/by-puuid/{Uri.EscapeDataString(bodyCore.Puuid)}";
        ...
        {
            var urlCore = $"{http.BaseAddress}riot/account/v1/accounts/by-puuid/{Uri.EscapeDataString(puuid)}";
            var resCore = await http.GetAsync(urlCore, ct);
            if (!resCore.IsSuccessStatusCode) return RiotHttpResult<RiotAccountResponse>.Fail((int)resCore.StatusCode);
            var bodyCore = await resCore.Content.ReadFromJsonAsync<RiotAccountCoreResponse>(ct);
            if (bodyCore == null) return RiotHttpResult<RiotAccountResponse>.Fail((int)HttpStatusCode.NotFound);
        ...
        public async Task<RiotHttpResult<List<string>>> GetMatchIdsAsync(string puuid, int count, CancellationToken ct)
        {
            var url = $"{http.BaseAddress}lol/match/v5/matches/by-puuid/{Uri.EscapeDataString(puuid)}/ids?start=0&count={count}";
            var res = await http.GetAsync(url, ct);
            if (!res.IsSuccessStatusCode) return RiotHttpResult<List<string>>.Fail((int)res.StatusCode);
        ...
        public async Task<RiotHttpResult<RiotMatchResponse>> GetMatchAsync(string id, CancellationToken ct)
        {
            var url = $"{http.BaseAddress}lol/match/v5/matches/{Uri.EscapeDataString(id)}";
            var res = await http.GetAsync(url, ct);
            if (!res.IsSuccessStatusCode) return RiotHttpResult<RiotMatchResponse>.Fail((int)res.StatusCode);

            var bodyRaw = await res.Content.ReadAsStringAsync(ct);
            var body = await res.Content.ReadFromJsonAsync<RiotMatchDto>(ct);
            if (body == null) return RiotHttpResult<RiotMatchResponse>.Fail((int)HttpStatusCode.NotFound);

            using var doc = JsonDocument.Parse(bodyRaw);
            var participantsJson = doc.RootElement.GetProperty("info").GetProperty("participants").GetRawText();


            // get match timeline 
            var urlTimeline = $"{http.BaseAddress}lol/match/v5/matches/{Uri.EscapeDataString(id)}/timeline";
            var resTimeline = await http.GetAsync(urlTimeline, ct);
            if (!resTimeline.IsSuccessStatusCode) return RiotHttpResult<RiotMatchResponse>.Fail((int)res.StatusCode);
